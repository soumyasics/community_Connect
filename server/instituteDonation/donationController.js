...
const InstituteDonationRequestModel = require("../instituteDonationRequests/donationRequestModel.js");
...
const createDonation = async (req, res) => {
  try {
    const { insId, requestId, donatedAmount } = req.body;
    if ((!insId, !requestId, !donatedAmount)) {
    ...
    const donationRequest = await InstituteDonationRequestModel.findById(requestId);
    if (!donationRequest) {
      return res.status(404).json({ message: "Request not found." });
    }
    ...
    institute.totalReceivedAmt += convertedDonatedAmt;
...
// all donations recieved to a single institute
const donationsToASingleInstitute = async (req, res) => {
  try {
    const id = req.params.id;
    if (!id) {
      return res.status(401).json({ message: "Id is required." });
    ...
    const donations = await DonationModel.find({ insId: id })
      .populate("insId")
      .populate("requestId")
    .populate("donatedOrganizationId")
      .populate("donatedUserId");
