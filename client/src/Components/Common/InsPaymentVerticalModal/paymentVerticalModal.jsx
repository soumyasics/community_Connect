...
export default function PaymentVerticalModal(props) {
  ...
  const collectData = (userBankAcDetails) => {
    if (donationReqData) {
      const insId = donationReqData.insId._id || null;
      console.log("ins iddd", insId);
  ...
  useEffect(() => {
    if (donationReqData) {
      setPaymentModalContent({
        heading: `Donate to ${donationReqData.insId?.name || "Orphanage"}`,
        ...
        content: `Pay ${
          donationReqData?.targetAmount -
            donationReqData.totallyCollectedAmount || 1000
        } rupees to the ${donationReqData?.insId?.name} orphanage`,
      });
    }
  }, [donationReqData]);
